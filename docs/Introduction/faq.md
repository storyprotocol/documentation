...
## _"My asset is off-chain. How do I register it as IP on Story Protocol?"_

You would mint an NFT that represents your off-chain asset, and register that NFT on Story.

## _"I am selling an off-chain asset (eg. merch). How do I make sure that automatic royalty flow is enforced?"_
...
## _“Why not be a protocol on every existing chain?”_
...
## _"Okay, so you need a singular blockchain. Why not use an existing blockchain?"_
...
## _"Why not just settle for an L2?"_
...
## _"Why is Story making improvements at the validator level?"_
...
## _"You mention needing an L1 to improve upon the efficiency of an IP graph. Why not build an L2 with off-chain graph indexing?"_
...
## _"How will Story prevent users from registering off-chain IP that isn't theirs?"_
